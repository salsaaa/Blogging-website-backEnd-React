      ...
      <div className="blog-entry">
        ...
        <div className="text p-4 d-block">
          <div className="meta mb-3">
            ...
            {localStorage.getItem('token') && <div><Link to={`/profile/${props.blog.userId}`} onClick={() => props.onMyProfile(false)} style={{ color: '#EF76AD', fontSize: '20px' }} >{props.blog.author}</Link></div>}
            <div><a className="meta-chat"><span className="icon-chat" /> 3</a></div>
          ...
          <h3 className="heading"><a ><p className="title">{props.blog.title}</p>{props.blog.body}</a></h3>
          <div className="row ">
            {props.blog.tags.map(
              tag => (
